...
description: Executes the complete four-step procedure for shipping a dev-store schema change in the superme repo. Use when a column is added, renamed, or removed in dev_store.py or the .dev.db schema — prevents silent migration no-ops and missing decoder updates that leave new fields invisible in API responses.
category: learned
---
...
1. **Edit `dev_store.py` — CREATE TABLE guard (adding a column).** Ensure the statement reads `CREATE TABLE IF NOT EXISTS`. Immediately after it, add an idempotent `ALTER TABLE … ADD COLUMN` guard wrapped in a try/except so it is a no-op when the column already exists:
   ...
   *For a rename:* SQLite does not support `ALTER TABLE … RENAME COLUMN` before v3.25. Use a migration: create a new column, copy data, update references in `_row_*`, then leave the old column (or recreate the table).
...
3. **Restart the dev daemon.** The daemon caches module imports at startup; code changes to `dev_store.py` are not picked up without a restart. Start command:
...
4. **Verify the new field in the response.** Hit the relevant endpoint and confirm the column appears:
