...
export type TraceRow = { kind: string; name?: string | null; description?: string | null; tool_id?: string | null }

export type PairedCall<T extends TraceRow> = { call: T; result: T | null }
...
export function pairTrace<T extends TraceRow>(events: T[]): PairedCall<T>[] {
  ...
  for (const e of events) {
    if (e.kind === 'result') {
      ...
      const slot: PairedCall<T> = { call: e, result: null }
      rows.push(slot)
      pending.push(slot)
    }
  }
